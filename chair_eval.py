...
def visualize(attn_map, title, cmap='viridis', save_dir=None, vmin=0, vmax=1): 
    plt.figure(figsize=(10, 8))
    sns.heatmap(attn_map, vmin=vmin, vmax=vmax, cmap=cmap)
    plt.xticks(ticks=[0.5, 5.5, 10.5, 15.5, 20.5, 25.5, 30.5], labels=[0, 5, 10, 15, 20, 25, 30])
    plt.xlabel('Head')
    plt.yticks(ticks=[0.5, 5.5, 10.5, 15.5, 20.5, 25.5, 30.5], labels=[0, 5, 10, 15, 20, 25, 30])
    plt.ylabel('Layer')
    plt.title(title)
...
class CHAIR(object):
    ...
    def compute_chair(self, ana_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            for idx, (word, node_word) in enumerate(zip(words, node_words)):
                ...
                if node_word not in gt_objects:
                    ...
                    # visualize(attn_img_mean, 'hall_img_mean', cmap='viridis', save_dir=save_dir)
                    ...
                    # visualize(attn_text_mean, 'hall_text_mean', cmap='viridis', save_dir=save_dir)
                    ...
                    # visualize(attn_img_mean, 'no_hall_img_mean', cmap='viridis', save_dir=save_dir)
                    ...
                    # visualize(attn_text_mean, 'no_hall_text_mean', cmap='viridis', save_dir=save_dir)
        ...
        # visualize(mean_hall_img_attn_mean, 'hall_img_mean', save_dir=os.path.join('results_analysis', ana_file + '_hall_img_mean.png'), vmax=0.7)
        # visualize(mean_nonhall_img_attn_mean, 'nonhall_img_mean')
        # visualize(mean_hall_text_attn_mean, 'hall_text_mean', save_dir=os.path.join('results_analysis', ana_file + '_hall_txt_mean.png'), vmax=0.7)
        # visualize(mean_nonhall_text_attn_mean, 'nonhall_text_mean')
